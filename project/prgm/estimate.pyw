...
class Drawer(Canvas):
    ...
    def draw(self, event=None, width=8):
        self.del_draw()
...
curve that is draw in the canvas"""
    def __init__(self, master, canvas, **arg):
        ...
        self.sc_nb_lines=Scale(f2, orient='horizontal', from_=1, to=20,\
                               resolution=1, tickinterval=5, length=400,\
                               label='Number of lines:', relief='ridge', bd=5,\
                               command=self.draw_lines)
        self.sc_nb_lines.grid(column=1, columnspan=3, row=1, padx=5, pady=5)
        self.sc_angle=Scale(f2, orient='horizontal', from_=1, to=10,\
                               resolution=1, tickinterval=3, length=450,\
                               label='Angle between lines: pi/?', \
                               relief='ridge', bd=5, command=self.draw_lines)
        self.sc_angle.grid(column=4, columnspan=3, row=1, padx=5, pady=5)
        #bt
        ...
        self.bt_erase_all.grid(row=2, column=1, pady=5, padx=5, sticky='e')
        #bt auto
        self.bt_auto=Button(f3, text='auto dots', fg='orange', bd=3, \
                            command=self.auto)
        self.bt_auto.grid(row=2, column=2, pady=5, padx=5, sticky='w')
        #bt erase
        self.bt_erase=Button(f3, text='Erase mode', command=self.erase_mode, \
                             fg='red', relief='sunken', bd=5, \
                             activeforeground='red')
        self.bt_erase.grid(row=3, column=1, pady=5, padx=5, sticky='e')
        #bt add
        self.bt_add=Button(f3, text='Add mode', command=self.add_mode, \
                           fg='green', relief='raised', bd=5, \
                           activeforeground='green')
        self.bt_add.grid(row=3, column=2, pady=5, padx=5, sticky='w')
        #lenght display
        self.lb_lenght=Label(f3, text='0 dot(s)\n\
length of the curve: 0.00 unit(s)', justify='left')
        self.lb_lenght.grid(row=1, column=1, columnspan=2, padx=10, pady=6)
        ...
        self.sc_unit_lenght=Scale(f4, orient='horizontal', from_=-15, to=45,\
                                  resolution=0.5, tickinterval=0, length=400,\
                                  label='unit length', relief='ridge', bd=5,\
...
length of the curve: '+str(round(l, 3))+'0 unit(s)')
        return l
...
if __name__=='__main__':
    ...
    root['menu']=helpbar
    #main drawer (canvas)
    can=Drawer(root, height=1200, width=1400, bg='white')
